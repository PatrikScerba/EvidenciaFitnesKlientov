...
public class ValidaciaVstupov {
    ...
    //Overí, či vek je číslo v rozsahu 0–120
    public static boolean jePlatnyVek(String textVek) {
        if (jePrazdne(textVek)) return false;
        if (!textVek.matches("\\d+")) return false;

        int vek = Integer.parseInt(textVek.trim());
